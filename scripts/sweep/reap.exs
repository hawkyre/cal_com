# The certification reaper: `Sweep.Reap` deletes anything a sweep left behind.
Code.require_file("client.exs", __DIR__)
Code.require_file("discover.exs", __DIR__)

Sweep.Client.start()

defmodule Sweep.Reap do
  @moduledoc """
  Finds and deletes certification fixtures by their name marker.

  Every fixture a sweep creates carries the marker, so a leftover can always be
  found again by listing the endpoint that owns it — even when a create's body
  did not parse and its id was never recorded. This is the backstop for
  `Sweep.Ledger.undo/1`.
  """
  ...
  @doc "Every place a fixture can live, and how to remove one."
  ...
  @doc "Delete every marked row; returns how many were removed."
  @spec reap(boolean()) :: non_neg_integer()
  def reap(apply?) do
  ...
  defp reap(credentials, {list_id, params, marked, delete_id, param}, apply?) do
    rows =
      case Sweep.Client.call(credentials, list_id, params) do
  ...
  defp call_quietly(credentials, operation_id, params) do
    case Sweep.Client.call(credentials, operation_id, params) do
      {:ok, _typed} -> :ok
